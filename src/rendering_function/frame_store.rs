...
pub(crate) struct FrameStore {
    ...
    pub(crate) secondary_command_buffers: Vec<CommandBuffer<{ SECONDARY }, { INITIAL }, { OUTSIDE }, true>>,
}
                            ...
                            |mut secondary_buffers| {
                                ...
                                secondary_buffers.iter_mut().for_each(|secondary_buffer| {
                                    secondary_buffer.cmd_set_viewport(&Viewport {
                                        x: 0.0,
                                        y: 0.0,
                                        width: self.renderpass_begin_info.render_area.extent.width as f32,
                                        height: self.renderpass_begin_info.render_area.extent.height as f32,
                                        min_depth: 0.0,
                                        max_depth: 1.0,
                                    });
